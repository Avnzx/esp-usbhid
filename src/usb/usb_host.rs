...
impl UsbHost {
    pub fn new(dm: p::PA11, dp: p::PA12, _usb: p::USB_OTG_FS) -> Self {
        unsafe {
    ...
    pub fn init(&self) {
        ...
        unsafe {
            // USB_CoreInit() from the SDK
            {
                ...
                REGS.hcfg()
                    .modify(|w| w.set_fslspcs(vals::Speed::FULL_SPEED));
            ...
            // USB_HostInit() from the SDK
            {
                ...
                {
                    assert!(
                        RX_FIFO_LEN + NON_PERIODIC_TX_FIFO_LEN + PERIODIC_TX_FIFO_LEN <= FIFO_LEN
                    );

                    REGS.grxfsiz().write(|w| w.set_rxfd(RX_FIFO_LEN));
